...
class SmallProgram implements ProgramType{
  ...
  shelf.Handler apiProxyHandler(innerHandler) => (shelf.Request request) { // 代理请求（后端服务）
    if (request.url.path.startsWith('api/')) return proxyHandler(serverAddress.toString())(request); // 代理请求(/api开头的请求)
    return innerHandler(request); // 非代理请求
  };
